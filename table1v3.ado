*** disclaimer ***
* This ado file has been modified by Jinying Chen to support data analysis for the HIT scoping review.
* The file is for internal research use only.

* modified the original table 1 function to add [frequency] option
...
capture program drop table1v3
program define table1v3
	version 12
	...
	* N
	...
	//only count each ref_ID once, modified by JChen, 2022-02-17
	egen tag_ref_ID = tag(ref_ID)
	drop if tag_ref_ID == 0
	//
		...
		if `"`arg'"' != "\" {
			...
			* categorical variable
			if "`vartype'"=="cat" | "`vartype'"=="cate" {
				...
				// added by JChen, handling zero observations, 2022-02-02
				qui count 
				if r(N) == 0 {
					restore
				...
				* finish restructuring to table1 format
				...
				else if "`freqonly'" =="freqonly" qui gen n_perc=string(_freq)
				else if "`isc'" == "isc" qui gen n_perc=string(_freq) + " (" + perc2 + ")"
				else qui gen n_perc=string(_freq) + " (" + perc + "%)"
				
				drop _freq tot perc perc2
